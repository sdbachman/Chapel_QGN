...
# Make directory
    ...
    f = open('run_transpose.sh')
    text = f.read()
    f.close()
    SF = re.compile('REPL1')
    command = './test_transpose -nl 1 --filename=' + base_dir + d + "/test_grid" + " --nx=" + str(nx) + " --ny=" + str(nx) + " --nz=" + str(nz) # + " &"
    print(command)
    new = SF.sub(command,text)
    f = open('run_transpose.sh', 'w')
    f.write(new)
    f.close()
